...
void CorrectPtRoch( const RoccoR _calib, const bool _doSys, const TLorentzVector _mu_vec, 
  ...
  } else if (_GEN_pt > 0) { q_term = _calib.kSpreadMC( _charge, _mu_vec.Pt(), _mu_vec.Eta(), _mu_vec.Phi(), _GEN_pt, 0, 0 );
  } else {                  q_term = _calib.kSmearMC(  _charge, _mu_vec.Pt(), _mu_vec.Eta(), _mu_vec.Phi(), _trk_layers, fRand_1, 0, 0 );
  }

  if ( fabs(q_term - 1.0) > 0.4 ) {
    std::cout << "\n*** BIZZARELY HIGH QTERM ***" << std::endl;
    std::cout << "GEN pT = " << _GEN_pt << ", RECO pT = " << _mu_vec.Pt() << ", Q term = " << q_term << ", fRand_1 = " << fRand_1 << std::endl;
    std::cout << "Layers = " << _trk_layers << ", charge = " << _charge << ", eta = " << _mu_vec.Eta() << ", phi = " << _mu_vec.Phi() << std::endl;
  }
  ...
  for (int i = 0; i < 100; i++) {
    ...
    } else if (_GEN_pt > 0) { q_term     = _calib.kSpreadMC( _charge, _mu_vec.Pt(), _mu_vec.Eta(), _mu_vec.Phi(), _GEN_pt, 1, i );
    } else {                  q_term     = _calib.kSmearMC(  _charge, _mu_vec.Pt(), _mu_vec.Eta(), _mu_vec.Phi(), _trk_layers, fRand_1, 1, i );
    }
